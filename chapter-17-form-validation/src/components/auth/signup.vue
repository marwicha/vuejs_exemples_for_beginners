...
import { required, email, numeric, minValue, minLength, sameAs, requiredUnless} from 'vuelidate/lib/validators'
  export default {
    ...
    validations: {
      //names here should be like from properties
      email: {
        required,
        email,
        //custom Validator async to see if email already exists
        unique: val => {
          if (val === '') return true
          return axios.get('/users.json?orderBy="email"&equalTo="' + val + '"')
                 .then(res => {
                   // return array of keys that object has
